...
void PMagicSquare::generateProblem() {
	...
	_constraints[_m-3] = new CEqual(liste, _magic, lDom, _taille);
		
	// DIAGONALE 2
	lDom = new Domain * [_taille];
	for(int i = 0; i < _taille; ++i) {
		lDom[i] = _domains[((i+1)*_taille)-1];
		cout << ((i+1)*_taille)-1 << endl;
	}
	_constraints[_m-2] = new CEqual(liste, _magic, lDom, _taille);
	...
	_constraints[_m-1] = new CAllDiff(lDom, _n);
